...
export const AdminPage: React.FC = () => {
  ...
  const user = useShallowReactive<ResponsePaginationData<Auth>>({
    data: [],
  ...
  const fetchData = (params?: GetAdminParams) => {
    const getParams = { ...params }
    if (!!filterParams.keyword) {
      getParams.keyword = filterParams.keyword
    }

    loading.promise(getAdminList()).then((response) => {
  ...
  const handleSubmit = (user: Auth) => {
    if (activeEditData.value) {
      ...
      submitting.promise(createAdmin(user)).then(() => {
        closeModal()
        refreshData()
      })
    }
