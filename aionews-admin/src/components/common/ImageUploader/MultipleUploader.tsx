  ...
  return (
    <div className={styles.multipleImageUploader}>
      {!fileUrls.length ? null : (
        <Button
          ...
          onClick={() => {
            ...
            message.info(`${fileUrls.length} address copied successfully`)
          }}
        >
          Copy the Markdown address of all images
        </Button>
      )}
      <Upload.Dragger
      ...
      >
        ...
        <p className="ant-upload-text">Drag the file here or click to upload</p>
        <br />
      </Upload.Dragger>
    </div>
  )
