...
import { prop, index, plugin, Ref, modelOptions, pre } from '@typegoose/typegoose'
import {
  IsString,
  IsBoolean,
  IsNotEmpty,
...
@pre<Article>('save', async function () { 
  const convertSlug = await this.title.toLowerCase().replace(/^\s+|\s+$/g, ''); // trim
  let slugConvert
  // remove accents, swap ñ for n, etc
  const from = "ãàáäâẽèéëêìíïîõòóöôùúüûñç·/_,:;";
  const to = "aaaaaeeeeeiiiiooooouuuunc------";
  for (let i = 0, l = from.length; i < l; i++) {
    slugConvert = await convertSlug.replace(new RegExp(from.charAt(i), 'g'), to.charAt(i));
  }
  this.slug = await slugConvert.replace(/[^a-z0-9 -]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-');
})
...
export class Article {
  ...
  @IsString()
  @IsOptional()
  @prop({unique:true, default: null, validate: /^[a-zA-Z0-9-_]+$/, index: true })
  slug: string
