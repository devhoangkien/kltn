...
export class DBBackupService {
  constructor(private readonly emailService: EmailService, private readonly awsService: AWSService) {
    logger.info('[expansion]', 'DB Backup Start the scheduled data backup task！')
  ...
  private doBackup() {
    return new Promise<UploadResult>((resolve, reject) => {
      ...
      shell.exec(`mongodump --uri="${MONGO_DB.uri}" --out="backup"`, (code, out) => {
        logger.info('[expansion]', 'DB Backup mongodump Execution complete!', code, out)
        if (code !== 0) {
        ...
        logger.info('[expansion]', 'DB Backup upload files: ' + fileName)
        logger.info('[expansion]', 'DB Backup file source location: ' + filePath)
