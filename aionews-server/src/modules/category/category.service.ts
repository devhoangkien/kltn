...
export class CategoryService {
  ...
  public async delete(categoryID: MongooseID) {
    ...
    // If there is no parent category for this category, delete{ pid: target.id } -> ok
    if (!categories.length) {
      return category
    }
    // Else recursively change -> { pid: target.id } -> { pid: target.pid || null }
  ...
  public async getAllCategories(): Promise<Array<Category>> {
    const allCategories = await this.categoryModel.find({pid: null}).lean().sort({ _id: SortType.Desc }).exec()
    const documents = await this.aggregate(true, allCategories)
    return documents
  }

  private async aggregate(publicOnly: boolean, documents: Array<Category>) {
    const counts = await this.articleModel.aggregate<{ _id: Types.ObjectId; count: number }>([
      { $match: publicOnly ? ARTICLE_LIST_QUERY_GUEST_FILTER : {} },
      { $unwind: '$category' },
      { $group: { _id: '$category', count: { $sum: 1 } } },
    ])
    const hydratedDocs = documents.map((category) => {
      const found = counts.find((item) => String(item._id) === String((category as any)._id))
