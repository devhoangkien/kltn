<template>
  <div class="archive-page" :class="{ mobile: isMobile }">
    <responsive>
      ...
      <template #mobile>
        <page-banner :position="70" :blur="false" :is-mobile="true" image="/images/page-archive/banner.jpg">
          <template #title>
    ...
    <div class="page-content">
      ...
      <div class="archive-warpper">
        <div class="container">
          <placeholder :data="archiveStore.archive?.articles.length" :loading="archiveStore.fetching">
                ...
                <ul class="month-list" v-for="node in archiveStore.tree" :key="node.year">
                  <li v-for="mn in node.months" :key="mn.month" class="month">
                    <h1 class="archive-title" :level="1">
                      <i18n>
                        <template #zh>
                          {{ numberToChinese(node.year) }}<span class="dot">•</span>{{ getMonthNameI18n(mn.month).zh
                          }}（{{ mn.articles.length }}）
                        </template>
                        <template #en>
                      ...
                      <li v-for="(article, index) in mn.articles" :key="index" class="article">
                        <div class="left">
                          <h3 class="article-title">
                            <span class="date">D{{ String(article.createAt.day).padStart(2, '0') }}</span>
                            <a class="link" target="_blank" :title="article.title"
