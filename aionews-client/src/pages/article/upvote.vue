<template>
  <div class="upvote">
    <div class="wrapper">
      <button class="button like" :class="{ liked: isLiked, parkinson: enabledParkinson, newliked: newliked }"
        :disabled="isLiked" @click="handleLike">
        <i class="icon iconfont icon-like"></i>
        <span class="text">
          <i18n>
            <template #zh>真棒！{{ likes }}</template>
            <template #en>{{ isLiked ? 'Upvoted' : 'Upvote' }} {{ likes }}</template>
            <template #vi>{{ isLiked ? 'Upvoted' : 'Upvote' }} {{ likes }}</template>
          </i18n>
        </span>
