    ...
    <div class="knowledge" key="knowledge">
      <div class="title">
        ...
        <div class="meta">
          ...
          <i18n>
            <template #zh>
              <span>共 {{ numberSplit(articleDetailStore.contentLength) }} 字，需阅读
                {{ articleDetailStore.readMinutes }} 分钟</span>
            </template>
            <template #en>
              <span>{{ numberSplit(articleDetailStore.contentLength) }} words,
                {{ articleDetailStore.readMinutes }} min read</span>
            </template>
            <template #vi>
              <span>{{ numberSplit(articleDetailStore.contentLength) }} từ,
                {{ articleDetailStore.readMinutes }} phút đọc</span>
            </template>
          </i18n>
      ...
      <transition name="module" mode="out-in">
        ...
        <markdown v-else-if="articleDetailStore.renderedFullContent" :html="articleDetailStore.moreContent?.html" />
      </transition>
    </div>
  </div>
</template>
