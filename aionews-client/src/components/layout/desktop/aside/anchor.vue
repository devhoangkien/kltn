<template>
  <div class="anchor" v-if="article">
    <button class="header" :title="article.title" @click="handleAnchor(ANCHORS.ARTICLE_CONTENT_ELEMENT_ID)">
      ...
      <div class="read">
        <i18n>
          <template #zh>共 {{ numberSplit(store.contentLength) }} 字，需阅读
            {{ store.readMinutes }} 分钟</template>
          <template #en>{{ numberSplit(store.contentLength) }} words, {{ store.readMinutes }} min
            read</template>
          <template #vi>{{ numberSplit(store.contentLength) }} từ, {{ store.readMinutes }} phút đọc
            </template>
        </i18n>
      </div>
    </button>
    <div class="catalogue" v-if="headings.length">
      <ul class="list" :class="`indent-${minHeadingLevel}`">
        <li class="item" :key="index" :title="heading.text" :class="`level-${heading.level}`"
          v-for="(heading, index) in headings" @click="handleAnchor(heading.id)">
          <i class="level iconfont" :class="`icon-h-${heading.level}`"></i>
          <span class="text">{{ heading.text }}</span>
        </li>
        <li class="item readmore" key="readmore" v-if="store.isLongContent && !store.renderedFullContent"
          :class="`level-${minHeadingLevel}`" @click="handleAnchor(ANCHORS.ARTICLE_READMORE_ELEMENT_ID)">
          <i class="level iconfont icon-loadmore"></i>
          <span class="text">
            <i18n :k="LanguageKey.ARTICLE_READ_ALL" />
          </span>
        </li>
      </ul>
    </div>
