<template>
  <div class="pen" :class="{ bordered }">
    <div class="markdown">
      <div ref="inputElement" class="markdown-input" :contenteditable="!disabled"
        :placeholder="t(LanguageKey.COMMENT_POST_PLACEHOLDER)" @focus="disableCopyrighter" @blur="enableCopyrighter" />
    ...
    <div class="pencilbox">
      <div class="stationery">
        ...
        <template v-if="!hiddenStationery">
          ...
          <button class="preview" title="preview" :class="{ actived: isPreviewed }" :disabled="disabled"
      ...
      <button type="submit" class="submit" :disabled="disabled" @click="handleSubmit">
        ...
        <i18n v-else-if="user.type === UserType.Local">
          ...
          <template #vi>Đăng với {{ user.localProfile?.name }}</template>
        </i18n>
        <i18n v-else-if="user.type === UserType.Disqus">
          ...
          <template #vi>Đăng với {{ user.disqusProfile?.name }}</template>
        </i18n>
        <i18n zh="发布" en="Publish" vi="Đăng" v-else />
