<template>
  <div class="tag">
    <placeholder :data="tagStore.sorted.length" :fetching="tagStore.fetching" :i18n-key="LanguageKey.TAG_PLACEHOLDER">
      <template #loading>
      ...
      <template #default>
        <div class="tag-list" key="list">
          <router-link class="item" :title="`${tagEnName(tag)} | ${tag.description}`" :to="getTagFlowRoute(tag.slug)"
            :key="index" v-for="(tag, index) in tagStore.sorted">
            <i class="iconfont" :class="getTagIcon(tag)" />
            <span class="name">
              <i18n :zh="tag.name" :en="tagEnName(tag)" :vi="tagViName(tag)" />
...
<script lang="ts">
  ...
  import { useTagStore, tagEnName, tagViName, Tag } from '/@/stores/tag'
