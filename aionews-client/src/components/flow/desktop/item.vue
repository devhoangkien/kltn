  ...
  <div class="article-item">
    ...
    <div class="item-content">
      <router-link class="item-thumb" :to="getArticleDetailRoute(article.id)">
        ...
        <div class="image" :style="{ backgroundImage: `url(${getThumbnailURL(article.thumb)})` }" :alt="article.title"
          :title="article.title" />
      </router-link>
      <div class="item-body">
        <div class="item-content">
        ...
        <div class="item-meta">
          ...
          <span class="categories">
            ...
            <placeholder :transition="false" :data="article.category.length">
              ...
              <template #default>
                <router-link v-for="(category, index) in article.category.slice(0, 1)" :key="index"
                  :to="getCategoryFlowRoute(category.slug)">
                  <i18n :zh="category.name" :en="category.slug" :vi="category.slug" />
                </router-link>
              </template>
            </placeholder>
          </span>
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const getLanguageText = (language: Language) => {
        return language === Language.Chinese ? '中文' : Language.English ? 'EN' : 'VI'
      }
