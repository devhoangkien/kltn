<template>
  <div class="articles">
    ...
    <div class="header" v-if="tagSlug || categorySlug || date || searchKeyword">
      <div class="content">
        ...
        <i18n v-if="searchKeyword">
          ...
          <template #vi>Từ khoá tìm kiếm  "{{ searchKeyword }}" </template>
        </i18n>
      </div>
    ...
    <!-- list -->
    <placeholder :data="articles.length" :loading="!articles.length && articleListStore.list.fetching">
      <template #loading>
      ...
      <template #default>
        <div>
          ...
          <!-- loadmore -->
          <div class="loadmore">
            <button class="button" :disabled="articleListStore.list.fetching || !hasMoreArticles"
...
<script lang="ts">
  ...
  import { useTagStore, tagEnName, tagViName } from '/@/stores/tag'
