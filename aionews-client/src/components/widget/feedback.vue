  ...
  <div class="feedback">
      ...
      <div class="submitted">
        ...
        <div class="text">
          <i18n>
            ...
            <template #vi>Cảm ơn bạn đã phản hồi.</template>
          </i18n>
        </div>
        <button class="close" @click="handleClose">
          <i18n zh="关闭窗口" en="Close" vi="Đóng" />
        </button>
      </div>
    </template>
    <template v-else>
      <div class="title">
        <i18n>
          ...
          <template #vi>Bạn đánh giá thế nào{{ META.title }}?</template>
        </i18n>
      </div>
      <div class="emotions">
        <li class="item" :key="emotion.value" v-for="emotion in emotions">
          <label>
            <input class="radio" type="radio" :value="emotion.value" :disabled="state.submitting"
            ...
            <div class="button" :class="{ activated: state.emotion === emotion.value }">
              ...
              <span class="text">
                <i18n :zh="emotion.zh" :en="emotion.en" :vi="emotion.vi" />
              </span>
            </div>
          </label>
      ...
      <div class="input">
        <textarea class="textarea" name="feedback" id="feedback" rows="10" v-model.trim="state.content"
          :disabled="state.submitting" :placeholder="
            isZhLang
              ? '你可在此畅所欲言，这将仅对博主可见'
              : isEnLang
              ? 'Tell me about your opinion...'
              : 'Nói cho tôi về ý kiến của bạn...'
          "></textarea>
        <div class="buttons">
          <button class="item cancel" :disabled="state.submitting" @click="handleClose">
            <span class="text">
              <i18n zh="取消" en="Cancel" vi="Huỷ" />
            </span>
          </button>
    ...
    <div class="history" v-if="historyFeedbacks.length">
      <i18n>
        ...
        <template #vi>Bạn có {{ historyFeedbacks.length }} lịch sử phản hồi.</template>
      </i18n>
    </div>
  </div>
</template>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(_, context) {
      const emotions = [
        { emoji: '😠', value: 1, en: 'Terrible', zh: '差劲', vi: 'Kinh khủng' },
        { emoji: '🙁', value: 2, en: 'Bad', zh: '不喜欢', vi: 'Tệ' },
        { emoji: '😐', value: 3, en: 'Normal', zh: '一般', vi: 'Bình thường' },
        { emoji: '🙂', value: 4, en: 'Good', zh: '好', vi: 'Tốt' },
        { emoji: '😄', value: 5, en: 'Great', zh: '非常好', vi: 'Tuyệt vời' }
      ]

      const metaStore = useMetaStore()
      const universalStore = useUniversalStore()
      const { gtag, isZhLang, isEnLang } = useEnhancer()
